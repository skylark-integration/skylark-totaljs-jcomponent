    ...
    self.make = function() {
      ...
      self.find('script').each(function() {
        ...
        switch (type) {
          case 'detail':
            var h = el.html();
            dcompile = self.view.helper.canCompile(h); //h.COMPILABLE();
      ...
      templates.empty && self.view.helper.canCompile(templates.empty) && COMPILE(eempty);
        ...
        if (eld.hclass(cls + '-detail')) {
          ...
          if (config.detail === true) {
            ...
            EXEC(config.detail, row, function(row) {
              ...
              if ((is && self.view.helper.canCompile(row)) || dcompile)
