  ...
  COMPONENT('contextmenu', function(self) {
    ...
    self.show = function(orientation, target, items, callback, offsetX, offsetY) {
      ...
      if (is) {
        ...
        var obj = target instanceof $ ? target[0] : target;
        if (self.target === obj) {
          self.hide(0);
          return;
        }
    ...
    $(document).on('click', 'a', function() {
      FIND('contextmenu').show('left', this, [{ name: 'Print', icon: 'fa-print', value: 'print' }, { name: 'Copy', icon: 'fa-copy', value: 'copy' }, { name: 'Remove', icon: 'fa-times-circle', value: 'remove' }], function(item) {
